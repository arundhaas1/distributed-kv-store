 ...
 *
 * Important caveat: every RPC here is a direct method call on the same JVM. Numbers are dominated by JIT, GC, and hashmap lookups — not network. Use them to compare relative trends (e.g. how lagging
 * peers affect latency), not as absolute "this Raft cluster does X ops/sec on real hardware" claims.
 */
public class RaftBenchmark {
	...
	private static void benchmarkThroughput() {
		...
		System.out.printf("  %,d ops in %.3f s → %,.0f ops/sec%n", THROUGHPUT_OPS, seconds, opsPerSec);
	}
	...
	private static void benchmarkLatency() {
		...
		System.out.printf("  p50 = %d µs   p95 = %d µs   p99 = %d µs   max = %d µs%n", p50 / 1000, p95 / 1000, p99 / 1000, max / 1000);
	}
	...
	private static void benchmarkElection() {
		...
		System.out.printf("  median = %d µs   p95 = %d µs   (over %d trials)%n", median / 1000, p95 / 1000, trials);
	...
	private static void benchmarkCatchup() {
		...
		System.out.printf("  caught up via %d heartbeat rounds in %.3f ms%n", heartbeats, elapsedNanos / 1_000_000.0);
		System.out.printf("  ≈ %.2f ms per heartbeat round%n", (elapsedNanos / 1_000_000.0) / heartbeats);
	}
